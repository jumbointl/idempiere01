...
class MInOutNotifier extends StateNotifier<MInOutStatus> {
  ...
  Future<LoadResult> loadMInOutAndLine(WidgetRef ref) async {
    debugPrint('[Notifier.loadMInOutAndLine] START type=${state.mInOutType} doc=${state.doc}');

    final doc = state.doc.trim();
    if (doc.isEmpty) {
      state = state.copyWith(isLoading: false, errorMessage: 'Documento vacío');
      return const LoadNotFound('Documento vacío');
    }
    ...
    if (isMInOutConfirmType(state.mInOutType)) {
      ...
      final savedConfirmId = ref.read(savedConfirmIdProvider);

      final mInOut = await getMInOutAndLine(ref);
      if (mInOut.id == null) {
        state = state.copyWith(isLoading: false, viewMInOut: false);
        return LoadNotFound('${Messages.NOT_M_IN_OUT_RECORD_FOUND} : $doc');
      }

      if (savedConfirmId > 0) {
        final m = await getMInOutConfirmAndLine(savedConfirmId, ref);
        state = state.copyWith(
          mInOutConfirm: m,
          mInOut: mInOut,
          isLoading: false,
      ...
      final confirmList = await getMInOutConfirmList(mInOut.id!, ref);
      state = state.copyWith(mInOutConfirmList: confirmList, isLoading: false);

      // ¿Auto-create o seleccionar?
      final shouldAutoCreate = isMInOutConfirmCreateFlow(state.mInOutType, confirmList);
      if (shouldAutoCreate) {
        // el Widget abrirá el bottomsheet de create, NO el notifier
        return LoadNeedAutoCreateConfirm(mInOut: mInOut, type: state.mInOutType);
      }

      return LoadNeedConfirmSelection(mInOut: mInOut, confirmList: confirmList);
    }
    ...
    if (isMoveConfirmType(state.mInOutType)) {
      ...
      if (savedConfirmId > 0) {
        final mInOut = await getMovementAndLine(ref);
        final mInOutConfirm = await getMovementConfirmAndLine(savedConfirmId, ref);
      ...
      if (mInOut.id == null) {
        state = state.copyWith(isLoading: false, viewMInOut: false);
        return LoadNotFound('Movimiento no encontrado: $doc');
      }

      final confirmList = await getMovementConfirmList(mInOut.id!, ref);
      state = state.copyWith(mInOut: mInOut, isLoading: false);

      if (confirmList.isEmpty) {
        return LoadNotFound('No hay confirmaciones disponibles para este movimiento.\n${mInOut.documentNo}');
      }

      return LoadNeedMoveConfirmSelection(mInOut: mInOut, confirmList: confirmList);
    }
  ...
  bool isMInOutConfirmCreateFlow(MInOutType type, List<MInOutConfirm> mInOutConfirmList) {
    ...
    bool result = type == MInOutType.shipmentConfirm ||
  ...
  Future<void> showSelectMInOutConfirm(
      BuildContext context,
                ...
                child: mInOutConfirmList.isNotEmpty
                  ...
                  itemBuilder: (context, index) {
                    ...
                    final bool canComplete =
                        item.isDraft || item.isInProgress;
                    ...
                    return InkWell(
                      onTap: () {
                        ...
                        if (!canComplete) {
                          String message =
                              '${Messages.DOCUMENT_STATUS} = ${item.docStatus.id}';
                        ...
                        if (mInOutState.mInOutType ==
                            MInOutType.moveConfirm) {
  ...
  Future<void> setDocActionConfirm(WidgetRef ref) async {
    ...
    try {
      ...
      if (listLinesToUpdateTargetQty.isNotEmpty) {
        ...
        for (final lc in listLinesToUpdateTargetQty) {
          debugPrint('setDocActionConfirm 4: mInOutRepository.updateLineConfirmTargetQty(');
          await mInOutRepository.updateLineConfirmTargetQty(lc, ref);
        }
      } else {
        progress.setStep(4, 'No hay borradores LineConfirm para actualizar.');
  ...
  void removeSavedMInOutData(WidgetRef ref) {
    ...
    try {
      ...
      if (filtered.isEmpty) {
        ...
        writeSavedPayloadList(box: box, key: key, list: filtered);
      }
...
// Un resultado para que el Widget sepa qué UI mostrar
sealed class LoadResult {
  const LoadResult();
}

class LoadOk extends LoadResult {
  const LoadOk();
}

class LoadNotFound extends LoadResult {
  final String message;
  const LoadNotFound(this.message);
}

class LoadNeedConfirmSelection extends LoadResult {
  final MInOut mInOut;
  final List<MInOutConfirm> confirmList;
  const LoadNeedConfirmSelection({required this.mInOut, required this.confirmList});
}

class LoadNeedAutoCreateConfirm extends LoadResult {
  final MInOut mInOut;
  final MInOutType type;
  const LoadNeedAutoCreateConfirm({required this.mInOut, required this.type});
}

class LoadNeedMoveConfirmSelection extends LoadResult {
  final MInOut mInOut;
  final List<MInOutConfirm> confirmList;
  const LoadNeedMoveConfirmSelection({required this.mInOut, required this.confirmList});
}
