...
final createShipmentConfirmProvider = FutureProvider.autoDispose<ResponseAsyncValue?>((ref) async {
  ...
  try {
    ...
    //final serviceType = 'RunGenerateShipmentupConfirm';
    //final columnName =  'AD_Record_ID';
    final request = ModelRunProcessRequest.runProcessCreateShipmentConfirmRequestJson(
    ...
    if (response.statusCode == 200) {
      ...
      if (runProcessResponse.isError == false) {
        ...
        String message = 'Error al crear los datos del shipment confirm: http status code 200,peron sin exito';
      ...
      responseAsyncValue.success = false;
      responseAsyncValue.data = null ;
      responseAsyncValue.message = 'Error al crear los datos del shipment confirm: http status ${response.statusCode}';
      print(responseAsyncValue.message);
      return responseAsyncValue;
