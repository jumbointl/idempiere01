...
class NewMovementLineCardState extends ConsumerState<NewMovementLineCard> {
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      ...
      ref.read(movementLineQuantityToMoveProvider(_lineId).notifier).state = initial;
    });
  ...
  void _attachManualListeners() {
    _deleteTriggerSub = ref.listenManual<DeleteRequest?>(
      deleteRequestProvider,
        ...
        ref.read(editingMovementLineProvider(_lineId).notifier).state = true;
        _deleteOpSub?.close();
      ...
      updateMovementLineIdProvider,
        ...
        _updateOpSub = ref.listenManual<AsyncValue<double?>>(
          editQuantityToMoveProvider(_lineId),
            ...
            n.whenOrNull(
              data: (result) {
                ref.read(updateMovementLineIdProvider.notifier).state = null;
                ref.read(quantityOfLineToEditProvider.notifier).state = null;
                ref.read(editingMovementLineProvider(_lineId).notifier).state = false;
                ...
                if (isSuccess) {
                  ...
                  ref.read(movementLineQuantityToMoveProvider(_lineId).notifier).state = result;
                  ...
                  final lines = List<IdempiereMovementLine>.from(m.movementLines ?? const []);
  ...
  Widget build(BuildContext context) {
    ...
    String att =
        '${widget.movementLine.mAttributeSetInstanceID?.identifier ?? '--'}'
    ...
    return AnimatedContainer(
      ...
      decoration: BoxDecoration(
        ...
        border: Border.all(
          color: widget.isHighlighted ? Colors.purple : Colors.transparent,
          width: widget.isHighlighted ? 5 : 0,
        ),
                        ...
                        onTap: () async {
                          ...
                          if (aux != null && aux >= 0) {
                            ...
                            if (aux == 0) {
                              ...
                              ref.read(quantityOfLineToEditProvider.notifier).state = [lineId, aux];
                              ref.read(updateMovementLineIdProvider.notifier).state = lineId;
                            }
                          } else {
                            String message =
                ...
                child: Column(
                  ...
                  children: [
                    Row(
                      children: [
                        Expanded(
                          child: Text('${widget.movementLine.id ?? '--'}', style: textStyleTitle),
                        ),
                        SizedBox(
                          width: 150,
                    ...
                    if (widget.showLocators ?? false)
                      Text(widget.movementLine.mLocatorID?.identifier ?? '--', style: textStyleTitle),
                    if (widget.showLocators ?? false)
                      Text(widget.movementLine.mLocatorToID?.identifier ?? '--', style: textStyleTitle),
