...
Future<void> printLabelZplWithQr(WidgetRef ref,String ip, int port,MovementAndLines movementAndLines) async {
  if(ip.isEmpty || port==0) {
    return;
  }
  print('PRINTING printLabelZplWithQr');
  ...
  String description =movementAndLines.description ??'';
  late var titleSizeWidth = PosTextSize.size1;
  int lines = movementAndLines.movementLines?.length ?? 0 ;
  double quantity = 0;
  for(int i = 0; i<lines; i++){
    quantity = movementAndLines.movementLines![i].movementQty ?? 0;
  }
  // 1. Cargar el perfil de la impresora
  final profile = await CapabilityProfile.load();
  final generator = Generator(PaperSize.mm80, profile); // Ajusta el tamaño del papel
...
Future<void> printZplLabel(WidgetRef ref, String ip, int port, List<int> ticket) async {
  print('PRINTING POS printPosTicket');
  final printer = PrinterNetworkManager(ip,
      port: port);
  PosPrintResult connect = await printer.connect();
